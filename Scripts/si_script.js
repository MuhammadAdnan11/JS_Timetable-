...
// subject adding dynamically start✅
...
function genTable(res) {
 ...
 // Build dynamic subject data from user inputs
 ...
 for (let i = 0; i < subjects.length; i++) {
   let subject = subjects[i].value.trim();
   let unit = parseInt(units[i].value);
   if (!subject || isNaN(unit) || unit <= 0) {
     alert("Please enter valid subject names and units.");
     return;
   }
   subjectArr.push({ subject, unit });
 }

 // Replace the hardcoded JSON with this:
 let myJ = { data: subjectArr };
 subData = myJ;
 res = myJ;
